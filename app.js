var createError = require('http-errors');
var express = require('express');
var path = require('path');
var cookieParser = require('cookie-parser');
var logger = require('morgan');

var userRouter = require('./routes/user');
var adminRouter = require('./routes/admin');
var hbs=require('express-handlebars') //we install this module by  using npm install express-handlebars for partialis
var fileUpdload= require("express-fileupload")//when has a file upload then we need a module that install by using "npm i express-fileupload"// and the create a object
var db=require('./config/connection')//take db file for call connect function when server start in app.js 
var app = express();
var session=require('express-session') //take session module after installation through npm i express-session

// view engine setup
app.set('views', path.join(__dirname, 'views'));
app.set('view engine', 'hbs');
//set a temblet engine
app.engine('hbs',hbs({extname:'hbs',defaultLayout:'layout',layoutsDir:__dirname+'/views/layouts/',partialsDir:__dirname+'/views/partials/'}))
app.use(logger('dev'));




app.use(express.json());
app.use(express.urlencoded({ extended: false }));
app.use(cookieParser());
app.use(express.static(path.join(__dirname, 'public')));
app.use(fileUpdload())//object creation for file upload
//use session
//secret key is the session component
//cookie handle take data // set time using maxAgae
app.use(session({secret:"Key",cookie:{maxAge:600000}}))

db.connect(function(err){
  if(err)
  console.log('Connection error'+err)
  else
  console.log('Database Connected...')
})

app.use('/', userRouter);
app.use('/admin', adminRouter);

// catch 404 and forward to error handler
app.use(function(req, res, next) {
  next(createError(404));
});

// error handler
app.use(function(err, req, res, next) {
  // set locals, only providing error in development
  res.locals.message = err.message;
  res.locals.error = req.app.get('env') === 'development' ? err : {};

  // render the error page
  res.status(err.status || 500);
  res.render('error');
});


module.exports = app;
